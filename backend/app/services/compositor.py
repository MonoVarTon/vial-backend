...
def extract_palette(image_bytes: bytes, colors: int = 3) -> list[tuple[int, int, int]]:
    ...
    try:
        ...
        # Сначала выбрасываем только полные пустышки: у однотонных картинок
        # квантование добивает палитру чистым чёрным, и это не цвет
        # изображения, а пустая ячейка. Порог низкий (15) сознательно —
        # отсекаем именно пустышки, а не тёмные цвета самой картинки.
        meaningful = [c for c in result if sum(c) >= 15]

        # И только теперь подтягиваем яркость. Порядок важен: если сначала
        # фильтровать по яркости, тёмно-синий из сожжённой NFT вылетит вместе
        # с пустышками и мы подставим цвета бренда — оттенок сожжённого
        # потеряется. Пропорциональная подтяжка сохраняет оттенок:
        # тёмно-синее остаётся синим, просто становится различимым.
        lifted = []
        for c in meaningful:
            if sum(c) < 180:
